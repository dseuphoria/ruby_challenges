...
def birth_path(birthdate)
	...
	number = number.to_s
	puts "The first step is adding the digits of your birthdate 
		which gives us #{number}"
	...
	puts "The next step is adding the digits in this number 
		which gives us #{number}. Your results are as follows: 
		"
		return number
end
...
def birth_path_explanation(number)
...
puts "What is your birthdate? (Must be in MMDDYYYY format.)"
birthdate = gets

number=birth_path(birthdate)

message=birth_path_explanation(number)
